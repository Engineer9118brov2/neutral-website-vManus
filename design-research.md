...
## Strict visual QA — duplicate identity correction

The first desktop audit exposed a duplicated Neutral identity lockup: the global header and the homepage hero each rendered a separate mark/wordmark. The hero lockup is now removed at all breakpoints, leaving the shared header as the single identity anchor. A follow-up desktop hero review confirms the duplicate has gone and the headline/atlas composition has recovered usable breathing room. The full mobile review confirms that Home, About, Products, and Contact render without duplicate titles, clipping, overlap, or blank transition artifacts. The mobile Products evidence flow remains stable after the record-layout correction; Contact’s support route and form hierarchy remain intact.
