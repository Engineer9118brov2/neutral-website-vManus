...
## Scrollytelling rebuild: documentation-grounded

**Narrative structure.** The page will become a continuous vertical demonstration rather than a sequence of decorative blocks. It starts with Neutral’s confirmed proposition—**One harness. All your models. No vendor lock-in.**—alongside a lightweight WebGL field. A pinned scroll chapter then turns one request into an explicit, visible route: **choose models, preserve local control, see the result.** The next chapter uses a bento grid to evidence confirmed Apple-side capabilities: agent memory, model endpoints, permission-aware tools, and device-local credentials. A final status chapter draws a sharp line between the working foundation and the roadmap: Electron is a desktop companion, while desktop tool execution, zero-setup Neutron, live billing, and advanced memory graphs remain future work.

**Interaction philosophy.** ScrollTrigger progresses a single signal across the page, choreographing opacity and transforms only; sections do not overlap via pseudo-elements, and the former glowing connector boxes are removed outright. A Three.js canvas sits as a pointer-events-none background layer, rendering a low-particle periwinkle orbital field and respecting reduced-motion preference. Parallax uses transforms with modest depth offsets. Bento cards reveal as a controlled stack, not as a dashboard wall. Mobile removes pinning and runs an accessible linear layout; reduced-motion turns every non-essential animation off.

## Style Decisions

- The constellation is not background decoration: it is a **semantic system map** with explicit nodes for the human request, model route, bounded context, permission-gated tool call, local boundary, and returned result. One visible signal traces that path across the hero and request chapter.
